%% Modèle SimpKcET : Chargement et mise en formes des fichiers netcdf FCOVER OLCI-300m
clc
clear all;

%% Import des fichiers

path = 'D:\Fcover\FCOVER_ncfile\';
files = dir(path);
...
%     complete_path = append(path,subfolder_name{i});
    full_Filename = fullfile(path,fileslist{i});
    variables = netcdf_info(full_Filename);
%     time_vec{i} = get_time_data(full_Filename);
    time_str{i} = extractBetween(full_Filename,45,52);
...
%     [index_long_pt2, index_lat_pt2] = get_coordinates_from1pt(filename_TP,5.8,43.9); % On récupère 1 index pour la latitude et la longitude
    FCOVER_FV{i} = extract_varFC300mOlci(index_long_pt,index_lat_pt,'FCOVER',full_Filename); % Extrait les données pour 1 pt précis
end
...
save('F_COVER_300m_Olci','F_COVER_TT');
