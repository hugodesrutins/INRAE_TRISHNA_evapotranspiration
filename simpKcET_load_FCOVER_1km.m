%% Modèle SimpKcET : Chargement et mise en formes des fichiers netcdf FCOVER 1Km
clc
clear all;

%% Import des fichiers

path = '\\147.100.1.28\Remote_Sensing\COPERNICUS_albert_final\fCOVER_vito\1km_vegetation\';
path_subfolder = dir(path);
path_subfolder = struct2table(path_subfolder);
subfolder_name = path_subfolder.name;
subfolder_name([1:2], :) = [];
fileslist = getAllFiles(path,'*.nc',0);
...
%     [index_long_pt2, index_lat_pt2] = get_coordinates_from1pt(filename_TP,5.8,43.9); % On récupère 1 index pour la latitude et la longitude
    FCOVER_FV{i} = extract_varFC(index_long_pt,index_lat_pt,'FCOVER',full_Filename); % Extrait les données pour 1 pt précis
...
save('F_COVER_1km_veg.mat','F_COVER_TT');
