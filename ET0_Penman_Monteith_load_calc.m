...
[Tair, Tmin, Tmax, esx, esm] = Get_t2m(t2m_global); 
[Dair, Dmin, Dmax,ea] = Get_d2m(d2m_global); 
...
Rn_idx1 = timerange("01-Jan-1959","01-Jan-2021");
Rn_Safran = Rn_Safran(Rn_idx1,:);


%% Calcul de ET0

ET0_ERA5_FAO = ET_Penman_Monteith_FAO(Delta,Tair,Rn_Safran.Rn',0,gamma,u2,es,ea); % ET0 FAO 56
ET0_ERA5_FAO = squeeze(ET0_ERA5_FAO);
ET0_ERA5_FAO = ET0_ERA5_FAO';

ET0_ERA5_MF = ET_Penman_Monteith_MF(Delta,Tair,Rn_Safran.Rn',0,gamma,raMF,es,ea); % ET0 Météo france
ET0_ERA5_MF = squeeze(ET0_ERA5_MF);
ET0_ERA5_MF = ET0_ERA5_MF';


%% Export Data and Complete timetable
...
Tair_TT =  array2timetable(Tair','RowTimes',date,'VariableNames',{'T_moy'});
Tmin_TT =  array2timetable(Tmin','RowTimes',date,'VariableNames',{'T_min'});
Tmax_TT=  array2timetable(Tmax','RowTimes',date,'VariableNames',{'T_max'});
Rns_TT = array2timetable(Rns','RowTimes',date,'VariableNames',{'Solar radiation shortwave'});
Rnl_TT = array2timetable(Rnl','RowTimes',date,'VariableNames',{'Thermal radiation longwave'});
u2_TT = array2timetable(u2','RowTimes',date,'VariableNames',{'Wind speed'});
gamma_TT = array2timetable(gamma','RowTimes',date,'VariableNames',{'gamma'});
ea_TT = array2timetable(ea','RowTimes',date,'VariableNames',{'ea'});

ET0_ERA5_FAO_MF_TT = [ET0_ERA5_FAO_TT  ET0_ERA5_MF_TT Tair_TT Tmin_TT Tmax_TT Rns_TT Rnl_TT u2_TT gamma_TT ea_TT];
...
save(fullfile(savepath3,'ea'),'ea');
save(fullfile(savepath3,'gamma'),'gamma');

save(fullfile(savepath2,'Tair_ERA5'),'Tair_TT');
save(fullfile(savepath2,'Tmin_ERA5'),'Tmin_TT');
save(fullfile(savepath2,'Tmax_ERA5'),'Tmax_TT');
save(fullfile(savepath2,'Tmax_ERA5'),'Tmax_TT');
save(fullfile(savepath2,'Rns_ERA5'),'Rns_TT');
save(fullfile(savepath2,'Rnl_ERA5'),'Rnl_TT');
